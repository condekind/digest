...
fn run_pattern_test(test: &PatternTest) -> Result<()> {
    ...
    for path_str in &test.paths_to_ignore {
        ...
        if !should_ignore(path, &patterns) {
            println!("❌ Error: Expected '{}' to be ignored, but it wasn't", path_str);
        } else {
    ...
    for path_str in &test.paths_to_include {
        let path = Path::new(path_str);
        if should_ignore(path, &patterns) {
            println!("❌ Error: Expected '{}' NOT to be ignored, but it was", path_str);
        } else {
...
fn main() -> Result<()> {
    let tests = vec![
        ...
        PatternTest {
            ...
            paths_to_include: vec![
                "builds/file.js".to_string(),
                "src/build.js".to_string(),
            ],
        },
